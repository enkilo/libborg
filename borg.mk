...
VERSION ?= 0$(warning no package version specified!)
...
ARFLAGS = rcs
GCCXML = gccxml
GCCXML_CONFIG = gccxml.conf
GCCXML_COMPILER = $(cross)g++-4.1.2
#GCCXML_CXXFLAGS = -I.
...
ifeq ($(DEPMODE),1)
postdep = && $(SED) -i 's,^\(.*\)\.o:,\1.o \1.d \1.ii:,' $*.d
endif
...
CC = $(cross)gcc$(if $(CC_VERSION),-$(CC_VERSION))
CXX = $(cross)g++$(if $(CXX_VERSION),-$(CXX_VERSION))
AR = $(cross)ar
NM = $(cross)nm
RANLIB = $(cross)ranlib
...
compiler-installdir = $(word 2,$(shell $1 -print-search-dirs))
compiler-installdirs = $(subst /,$(space),$(call compiler-installdir,$1))

add-until = $(if $1,$(if $(subst $1,,$(firstword $2)),$(firstword $2) $(call add-until,$1,$(wordlist 2,$(words $2),$2))))

root := /$(subst $(space),/,$(strip $(call add-until,lib,$(call compiler-installdirs,$(CXX)))))
prefix := $(patsubst %/$(host),%,$(root))
...
# Warning level.
ifneq ($(WARNINGS),)
WARNFLAGS += $(addprefix -W,$(WARNINGS))
endif
...
ifneq ($(OPTIMIZE),0)
ifneq ($(MACHINE),)
MACHFLAGS += $(addprefix -m,$(MACHINE))
endif
endif

CFLAGS = $(DBGFLAGS) $(OPTFLAGS) $(DEPFLAGS) $(MACHFLAGS) $(WARNFLAGS)
...
XMLCOMPILE = $(call exec-var,GCCXML$(space),$(if $(GCCXML_COMPILER), --gccxml-compiler "$(GCCXML_COMPILER)")$(if $(GCCXML_CXXFLAGS), --gccxml-cxxflags "$(GCCXML_CXXFLAGS)")$(if $(CPPFLAGS), $(CPPFLAGS)))

# Global build configuration.
# ---------------------------------------------------------------------------
#DEPFILES = $(subst .o,.d,$(OBJECTS))
XMLFILES = $(patsubst %.cpp,%.xml,$(wildcard *.cpp))
...
$(outdir)%.$(slibext) lib%.$(ilibext):
	echo 'target: $@'
	$(call exec-var,CXXLINK,$(call var-strip,LDFLAGS)$(if $(DRIVER), --driver-name=$(DRIVER))$(if $(filter %.def,$^ $|), --def=$(filter %.def,$^ $|),$(if $(wildcard lib$(notdir $*).def), --def=lib$(notdir $*).def, --export-all-symbols)) \
		$(if $(filter %.$(ilibext),$@),--output-lib=$@) -o $(outdir)$*.$(slibext) $(filter-out %.def,$^) $(LDADD) $(LIBS),$(if $(outdir),,$(outdir))$*.$(slibext),$@)
endif
...
%$(binext): #%.cpp
	$(if $(CCLD),$(CCLD),$(LINK)) $(CPPFLAGS) -o $@ $^ $(filter %.a,$|)$(if $(strip $(LDADD)), $(LDADD))$(if $(strip $(LIBS)), $(LIBS))
...
ifeq ($(SUBDIRS)$(AUXDIRS)$(SRCDIRS),)
	$(call exec-var,MKDIR$(pad-short),$(outdir)$(DISTDIR),$(DISTDIR))
else
ifeq ($(Q-$(echo-mode)),)
	$(call exec-var,MKDIR$(pad-short),$(addprefix $(outdir)$(DISTDIR),. $(AUXDIRS) $(SRCDIRS)))
else
	@for dir in . $(AUXDIRS) $(SRCDIRS); do \
...
#else
	@for file in $(filter-out $(AUXDIRS) $(SRCDIRS),$(DISTFILES)); do \
...
GETDIRS = $(wildcard $(addsuffix /,$(patsubst %/,%,$(AUXDIRS) $(SRCDIRS))))
