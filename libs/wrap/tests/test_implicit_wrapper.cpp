...
//  bigger_than_word s2 = { 1.0, -1.0 };
  ...
  wrapper<const some_object *> wptr_constobj(&obj1);
  ...
  // Wrap as <some_object *>
  wrapper<some_object *> wptr_obj(&obj1);
  wptr_obj.set(&obj2);
  some_object *ptr_obj = wptr_obj.get();
  ...
  wrapper<some_object *const> wconstptr_obj = &obj1;
  some_object *const constptr_obj = wconstptr_obj;

  BOOST_CHECK(!wconstptr_obj.is_mutable);
  BOOST_CHECK_EQUAL(constptr_obj->sum(), -1.0);
  
  // Wrap as <const some_object &>
  wrapper<const some_object &> wref_constobj = obj1;
  const some_object &ref_constobj = wref_constobj;

  BOOST_CHECK(!wref_constobj.is_mutable);
  BOOST_CHECK_EQUAL(ref_constobj.sum(), -1.0);
  
  // Wrap as <some_object &>
  wrapper<some_object &> wref_obj = obj1;
  some_object &ref_obj = wref_obj;    

  BOOST_CHECK(!wref_obj.is_mutable);
  BOOST_CHECK_EQUAL(ref_obj.sum(), -1.0);
  
  // Copy constructible?
  wrapper<some_object &> wref2_obj = wref_obj;
  some_object &ref2_obj = wref2_obj;

  BOOST_CHECK(!wref2_obj.is_mutable);
  BOOST_CHECK_EQUAL(ref2_obj.sum(), -1.0);
  
  // Copy constructible?
  wrapper<some_object *> wptr2_obj = wptr_obj;
  wptr2_obj.set(&obj2);
  some_object *ptr2_obj = wptr2_obj;

  BOOST_CHECK(wptr2_obj.is_mutable);
  BOOST_CHECK_EQUAL(ptr2_obj->sum(), 1.0);

  // Copy constructible ...?
/*  wrapper<const some_object *> wconstptr2_obj(wptr_obj);
  const some_object *constptr2_obj = wconstptr2_obj;
  ...
  wrapper<some_object *> wptr3_obj = wconstptr_obj;
  wptr3_obj.set(&obj1);
  some_object *ptr3_obj = wptr3_obj;

  BOOST_CHECK(wptr3_obj.is_mutable);
  BOOST_CHECK_EQUAL(ptr3_obj->sum(), -1.0);
  
  // Copy constructible from mutable when mutable?
  wrapper<some_object *> wptr4_obj = wptr3_obj;
  some_object *ptr4_obj = wptr4_obj;
...
//  test_mutable_wrapper<bigger_than_word>(s1, s2);
}
